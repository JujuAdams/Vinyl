...
|`parent`        |Set implicitly by a pattern that caused a sound to be played e.g. a voice of a Multi pattern is the parent of each child sound that is concurrently playing for that pattern                                                                                                                    |
...
|`emitter`       |Implicitly set by calculating the distance from the listener to the emitter that a Vinyl voice is playing on. Only Vinyl voices created by [`VinylPlayOnEmitter()`](Emitter-Functions) will factor in emitter gain, otherwise this gain is ignored                                              |
