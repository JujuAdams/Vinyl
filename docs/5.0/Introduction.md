...
If you take a look at the sidebar, the API is split across multiple different features. Each page lists functions relevant to that feature and details any extra contextual information that may be useful. Where I've had time I've also added example code to make things a bit quicker to pick up. For some of the chunkier topics - [gain](gain) being a notable example because boy that get complex fast - there are separate deep dive pages. These can be found in the sidebar under the **Glossary** heading.
