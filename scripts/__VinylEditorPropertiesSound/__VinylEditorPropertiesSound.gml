...
function __VinylEditorPropertiesSound(_soundName, _soundData, _modified, _defaultData, _selectionHandler, _patternDict)
{
    ...
    if (_soundName != __VINYL_FALLBACK_NAME)
    {
        ...
        {
            var _projectSoundDict = __VinylDocument().__GetProjectSoundDictionary();
            var _projectSound = _projectSoundDict[$ _soundName];
                ...
                repeat(array_length(_audioGroupArray))
                {
                    ...
                    if (ImGui.Selectable(_audioGroup, _projectSound.__audioGroup == _audioGroup))
                    {
                        _selectionHandler.__ForEachSelected(method({
                            __projectSoundDict: _projectSoundDict,
                            __audioGroup: _audioGroup,
                        }, function(_name)
                        {
                            var _projectSound = __projectSoundDict[$ _name];
                            if (_projectSound != undefined) _projectSound.__SetAudioGroup(__audioGroup);
                        }));
                    }
            ...
            if (ImGui.BeginCombo("##Asset Tag", __VinylAssetTagsGetAbbreviatedName(_soundAssetTags), ImGuiComboFlags.None))
            {
                ...
                repeat(array_length(_assetTagArray))
                {
                    ...
                    if (_oldValue != _newValue)
                    {
                        ...
                        {
                            var _projectSound = __projectSoundDict[$ _name];
                            if (_projectSound != undefined) _projectSound.__SetAssetTag(__assetTag, __newValue);
                        }));
                    }
            ...
            repeat(array_length(_compressionArray))
            {
                ...
                if (ImGui.RadioButton(_compressionArray[_i], (_projectSound.__attributes == _i)))
                {
                    ...
                    {
                        var _projectSound = __projectSoundDict[$ _name];
                        if (_projectSound != undefined) _projectSound.__SetAttributes(__attributes);
                    }));
                }
