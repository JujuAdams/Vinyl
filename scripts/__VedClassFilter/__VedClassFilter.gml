...
function __VedClassFilter() constructor
{
    ...
    static __Apply = function(_struct)
    {
        ...
        if (__useAudioGroup)
        {
            if (not variable_struct_exists(__audioGroupDict, _struct.__audioGroup)) return __invert;
        }
        ...
        if (__useAttribute)
        {
            if (not __attributesArray[_struct.__attributes]) return __invert;
        }
    ...
    static __BuildUI = function()
    {
        if (ImGui.BeginTable("Table", 3))
        {
            ...
            __invert = ImGui.Checkbox("##Filter Invert Checkbox", __invert);
