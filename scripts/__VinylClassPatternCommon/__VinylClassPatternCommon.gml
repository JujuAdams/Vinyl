    ...
    static __InitializeAssetArray = function(_assetArray)
    {
        ...
        repeat(array_length(__assetArray))
        {
            ...
            else
            {
                if (is_string(_asset))
                {
                    if (VinylAssetGetIndex(_asset) < 0) __VinylError("Error in ", self, " for \"assets\" property\nAsset \"", _asset, "\" not found in the project");
                    if (VinylAssetGetType(_asset) != asset_sound) __VinylError("Error in ", self, " for \"assets\" property\nAsset \"", _asset, "\" not a sound asset");
